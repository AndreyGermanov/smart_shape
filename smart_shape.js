...
const a = new at(), lt = (t) => t * (Math.PI / 180), pt = (t) => t * (180 / Math.PI), P = (t, e, i, s, o) => {
  ...
  const i = (p, d, c) => d.x <= Math.max(p.x, c.x) && d.x >= Math.min(p.x, c.x) && d.y <= Math.max(p.y, c.y) && d.y >= Math.min(p.y, c.y), s = (p, d, c) => {
    let b = (d[1] - p[1]) * (c[0] - d[0]) - (d[0] - p[0]) * (c[1] - d[1]);
    return b === 0 ? 0 : b > 0 ? 1 : 2;
  }, o = (p, d, c, b) => {
    let v = s(p, d, c), D = s(p, d, b), x = s(c, b, p), B = s(c, b, d);
    return v !== D && x !== B || v === 0 && i(p, c, d) || D === 0 && i(p, b, d) || x === 0 && i(c, p, b) ? !0 : !!(B === 0 && i(c, d, b));
  };
  if (t.length < 3)
    return !1;
  ...
  return i === z.CURRENT_TO_ORIGINAL ? ([t, e] = k(t, e, p, d, s), t -= h, e -= r, t *= 1 / o, e *= 1 / n) : i === z.ORIGINAL_TO_CURRENT && (t *= o, e *= n, t += h, e += r, [t, e] = k(t, e, p, d, s)), [t, e];
...
function se(t) {
  this.resizeBox = t, this.subscriptions = {
    ...
    this.resizeBox.redraw(), a.emit(l.POINT_DRAG_END, this.resizeBox, A(e, { point: e.target })), a.emit(w.RESIZE_BOX_RESIZE, this.resizeBox, A(e, { oldPos: i, newPos: s }));
  }, this.onLeftTopDragMove = (e) => {
...
function oe(t) {
  this.shape = t, this.subscriptions = {
    ...
    !this.shape.resizeBox || (this.resizeBoxListener = this.shape.resizeBox.addEventListener(w.RESIZE_BOX_RESIZE, this.onResize), this.resizeMouseDownEventListener = this.shape.resizeBox.addEventListener(l.SHAPE_MOVE_START, this.mousedown), this.resizeMouseMoveEventListener = this.shape.resizeBox.addEventListener(l.SHAPE_MOUSE_MOVE, this.mousemove), this.resizeClickEventListener = this.shape.resizeBox.addEventListener(l.SHAPE_MOUSE_CLICK, this.click), this.resizeDblClickEventListener = this.shape.resizeBox.addEventListener(l.SHAPE_MOUSE_DOUBLE_CLICK, this.svg_dblclick), this.resizeMouseDownEventListener = this.shape.resizeBox.addEventListener(l.SHAPE_MOUSE_DOWN, this.svg_mousedown), this.resizeMouseOverEventListener = this.shape.resizeBox.addEventListener(l.SHAPE_MOUSE_OVER, this.svg_mouseover), this.resizeMouseOutEventListener = this.shape.resizeBox.addEventListener(l.SHAPE_MOUSE_OUT, this.svg_mouseout), this.resizeMouseUpEventListener = this.shape.resizeBox.addEventListener(l.SHAPE_MOUSE_UP, (e) => {
      a.emit(l.SHAPE_MOUSE_UP, this.shape, A(e));
    ...
    et(e), a.emit(l.SHAPE_MOUSE_DOWN, this.shape, A(e)), setTimeout(() => {
      a.emit(
        l.SHAPE_MOVE_START,
        this.shape,
    ...
    if (this.shape.draggedPoint || a.emit(l.SHAPE_MOUSE_MOVE, this.shape, A(e)), e.buttons !== 1)
      return;
    ...
    a.emit(l.SHAPE_MOVE, this.shape, A(e, { oldPos: o, newPos: n }));
  }, this.mouseenter = (e) => {
    a.emit(l.SHAPE_MOUSE_ENTER, this.shape, A(e));
  }, this.mouseover = (e) => {
    f.draggedShape !== this.shape && a.emit(l.SHAPE_MOUSE_OVER, this.shape, A(e));
  }, this.mouseout = (e) => {
    a.emit(l.SHAPE_MOUSE_OUT, this.shape, A(e));
  }, this.click = (e) => {
    a.emit(l.SHAPE_MOUSE_CLICK, this.shape, A(e));
  }, this.doubleclick = (e) => {
    a.emit(l.SHAPE_MOUSE_DOUBLE_CLICK, this.shape, A(e));
...
function ne() {
  this.draw = (t) => {
    ...
    if (!t.rotateBox)
      return;
    const e = t.getResizeBoxBounds();
    t.options.displayMode === u.ROTATE ? t.rotateBox.options.shapeOptions.visible = t.options.visible : t.rotateBox.options.shapeOptions.visible = !1, t.rotateBox.left = e.left, t.rotateBox.top = e.top, t.rotateBox.width = e.width, t.rotateBox.height = e.height, t.rotateBox.options.zIndex = t.options.zIndex + 1, t.rotateBox.redraw(), t.rotateBox.shape.points.forEach((i) => {
      i.options.zIndex = t.options.zIndex + 2, i.element.style.zIndex = t.options.zIndex + 2;
    });
    ...
    const p = document.createElement("div");
    p.appendChild(r);
    const d = p.innerHTML, c = new Image(), b = new Blob([d], { type: "image/svg+xml" }), v = window.URL || window.webkitURL || window, D = await Z(b);
...
const H = {
  ...
  for (let s of e) {
    ...
    for (let c of o) {
      const [b, v] = [c[0], -c[1]];
      b < h && (h = b), b > r && (r = b), v < p && (p = v), v > d && (d = v), n.push({ x: b, y: v });
    }
...
function de() {
  this.shapes = {}, this.visibleShapes = {}, this.activeShape = null, this.draggedShape = null, this.shapeOnCursor = null, this.containerEventListeners = [], this.init = () => (this.setEventListeners(), this), this.setEventListeners = () => {
    ...
    this.shapeOnCursor && this.shapeOnCursor !== s && this.shapeOnCursor.getShapeSvg() && (this.shapeOnCursor.getShapeSvg().style.cursor = "default", this.shapeOnCursor.eventListener.mouseout(A(t, { target: this.shapeOnCursor }))), s && s !== this.shapeOnCursor && s.eventListener.mouseover(A(t, { target: s })), this.shapeOnCursor = s, this.shapeOnCursor && (a.emit(l.SHAPE_MOUSE_MOVE, this.shapeOnCursor, A(t)), this.shapeOnCursor.getShapeSvg().style.cursor = "crosshair");
...
function ht() {
  this.left = 0, this.top = 0, this.right = 0, this.bottom = 0, this.width = 0, this.height = 0, this.shape = null, this.guid = N(), this.options = {
    ...
    zIndex: 1e3
  }, this.eventListener = null, this.left_top = null, this.left_bottom = null, this.right_top = null, this.right_bottom = null, this.init = (t, e, i, s, o, n = {}) => (this.left = parseInt(e), this.top = parseInt(i), this.width = parseInt(s), this.height = parseInt(o), this.right = this.left + this.width, this.bottom = this.top + this.height, this.setOptions(n), this.options.shapeOptions.id = this.options.id, this.options.shapeOptions.canRotate = !1, this.options.shapeOptions.canScale = !1, this.shape = new R().init(t, S({}, this.options.shapeOptions), []), a.emit(l.SHAPE_CREATE, this.shape, {}), this.options.shapeOptions.pointOptions.bounds = this.shape.getBounds(), this.addPoints(), this.eventListener = new ie(this).run(), this.redraw(), this), this.setOptions = (t = {}) => {
...
function rt() {
  this.left = 0, this.top = 0, this.right = 0, this.bottom = 0, this.width = 0, this.height = 0, this.shape = null, this.guid = N(), this.options = {
    ...
    zIndex: 1e3,
    onlyMove: !1
  }, this.eventListener = null, this.left_top = null, this.left_center = null, this.left_bottom = null, this.center_top = null, this.center_bottom = null, this.right_top = null, this.right_center = null, this.right_bottom = null, this.init = (t, e, i, s, o, n = {}) => (this.left = parseInt(e), this.top = parseInt(i), this.width = parseInt(s), this.height = parseInt(o), this.right = this.left + this.width, this.bottom = this.top + this.height, this.setOptions(n), this.options.shapeOptions.id = this.options.id, this.options.shapeOptions.canRotate = !1, this.options.shapeOptions.canScale = !1, this.shape = new R().init(t, S({}, this.options.shapeOptions), []), a.emit(l.SHAPE_CREATE, this.shape, {}), this.options.shapeOptions.pointOptions.bounds = this.shape.getBounds(), this.addPoints(), this.eventListener = new se(this).run(), this.redraw(), this), this.setOptions = (t = {}) => {
    !t || typeof t != "object" || (this.options = S(this.options, t), this.options.shapeOptions.zIndex = this.options.zIndex || 1e3, this.options.shapeOptions.id = this.options.id || "", this.shape && this.shape.setOptions(this.options.shapeOptions));
  }, this.addPoints = () => {
...
try {
  window.ResizeBox = rt, window.SmartShape = R, window.RotateBox = ht, window.SmartShapeManager = f, window.SmartShapeGroupHelper = nt, window.SmartShapeDisplayMode = u, window.ShapeEvents = l, window.createEvent = A, window.getMousePos = Q, window.getMouseCursorPos = W;
} catch {
}
export {
  a as EventsManager,
