...
const h = new Z(), J = (t) => t * (Math.PI / 180), X = (t) => t * (180 / Math.PI), R = (t, s, e, i, o) => {
  const n = J(t), a = (s - i) * Math.cos(n) - (e - o) * Math.sin(n) + i, A = (s - i) * Math.sin(n) + (e - o) * Math.cos(n) + o;
  return [a, A];
}, I = (t, s, e, i) => Math.sqrt(Math.pow(e - t, 2) + Math.pow(i - s, 2)), K = (t, s) => {
  const e = (p, l, g) => l.x <= Math.max(p.x, g.x) && l.x >= Math.min(p.x, g.x) && l.y <= Math.max(p.y, g.y) && l.y >= Math.min(p.y, g.y), i = (p, l, g) => {
    let S = (l[1] - p[1]) * (g[0] - l[0]) - (l[0] - p[0]) * (g[1] - l[1]);
    return S === 0 ? 0 : S > 0 ? 1 : 2;
  }, o = (p, l, g, S) => {
    let _ = i(p, l, g), m = i(p, l, S), M = i(g, S, p), P = i(g, S, l);
    return _ !== m && M !== P || _ === 0 && e(p, g, l) || m === 0 && e(p, S, l) || M === 0 && e(g, p, S) ? !0 : !!(P === 0 && e(g, l, S));
  };
  if (t.length < 3)
    return !1;
  ...
  return a % 2 === 1;
}, H = (t, s, e, i) => !t && !s || !e || !i ? [e, i] : t && s ? [t, s] : (t || (t = s * (e / i)), s || (s = t * (i / e)), [t, s]), L = (t, s = !0) => {
  let e = 0, i = 0;
  if (!s)
  ...
  return !1;
}, b = (t) => typeof t < "u" && t !== null, C = (t, s) => t && typeof t == "object" && s && typeof s == "object" ? Object.assign(t, s) : t, q = (t) => {
...
function it(t, s, e = null) {
  this.panel = null, this.container = s, this.items = t, this.event = e || "contextmenu", this.listeners = {}, this.origEvent = null, this.cursorX = 0, this.cursorY = 0, this.overflowY = "", this.maxImageHeight = 0, this.subscriptions = {}, this.init = () => (Object.assign(this, new st(this)), this.container.addEventListener(this.event, (i) => (this.onEvent(i), !1)), x.emit(B.CREATE, this, { owner: this }), this), this.onEvent = (i) => {
    ...
    for (let o of i) {
      ...
      A.style.marginRight = "5px", A.style.display = "flex", A.style.flexDirection = "row", A.style.justifyContent = "center", A.style.alignItems = "center", n.height = this.panel.querySelector("#" + o.id).clientHeight, n.height > this.maxImageHeight && (this.maxImageHeight = n.height), n.style.verticalAlign = "middle", n.style.display = "", A.appendChild(n), this.panel.querySelector("#" + o.id + " div") || this.panel.querySelector("#" + o.id).insertBefore(A, a);
    }
    ...
    const n = (a) => {
      !this.origEvent || (x.emit(i, this.origEvent.target, et(a, {
    ...
    if (!this.container || (x.emit(B.SHOW, this, { owner: this }), this.drawMenu(), !this.panel))
      return;
    ...
    this.panel && (this.panel.innerHTML = ""), this.panel = null, x.emit(B.DESTROY, this, { owner: this });
  };
}
const B = {
  CREATE: "create",
...
function Ct() {
  return this.options = {
    ...
    hidden: !1,
    forceDisplay: !1
  }, this.x = 0, this.y = 0, this.element = null, this.guid = D(), this.subscriptions = {}, this.init = (t, s, e = null) => (this.x = parseInt(t), this.y = parseInt(s), Object.assign(this, new yt(this)), this.element = this.createPointUI(), this.setOptions(Object.assign({}, e)), this.setEventListeners(), h.emit(c.POINT_ADDED, this), this), this.setOptions = (t) => {
    ...
    h.emit(c.POINT_MOUSE_DOWN, this, d(t)), t.buttons === 1 && this.options.canDrag && (h.emit(c.POINT_DRAG_START, this, d(t)), G(t));
  }, this.mousemove = (t) => {
    if (h.emit(c.POINT_MOUSE_MOVE, this, d(t)), t.buttons !== 1 || !this.options.canDrag || !E.draggedShape || E.draggedShape.draggedPoint !== this)
      return;
    const s = this.x, e = this.y, i = L(this.element.parentNode, !0);
    if (!this.checkFitBounds(this.x + t.movementX, this.y + t.movementY)) {
      h.emit(c.POINT_DRAG_MOVE, this, d(t, { oldX: s, oldY: e }));
      return;
    }
    let o = t.clientX + window.scrollX - i.left - this.options.width / 2, n = t.clientY + window.scrollY - i.top - this.options.height / 2;
    [o, n] = this.applyMoveRestrictions(o, n, s, e), this.x = o, this.y = n, this.element.style.left = this.x + "px", this.element.style.top = this.y + "px", h.emit(c.POINT_DRAG_MOVE, this, d(t, { oldX: s, oldY: e }));
  }, this.mouseover = (t) => {
    h.emit(c.POINT_MOUSE_OVER, this, d(t));
  }, this.mouseout = (t) => {
    h.emit(c.POINT_MOUSE_OUT, this, d(t));
  }, this.click = (t) => {
    h.emit(c.POINT_MOUSE_CLICK, this, d(t));
  }, this.doubleclick = (t) => {
    h.emit(c.POINT_MOUSE_DOUBLE_CLICK, this, d(t));
    ...
    h.emit(c.POINT_MOUSE_UP, this, d(t)), t.button !== 2 && h.emit(c.POINT_DRAG_END, this, d(t));
...
function Bt(t) {
  this.rotateBox = t, this.subscriptions = {
    ...
    if (a === null)
      return;
    let A = a;
    this.previousAngle && (A -= this.previousAngle), this.previousAngle = a, h.emit(T.ROTATE_BOX_ROTATE, this.rotateBox, { angle: A });
  }, this.calcAngle = (s, e, i, o) => {
    const n = this.calcHypotenuse(s, e, i, o);
    if (n <= 0)
      return null;
    const a = this.calcCathetus(s, e, i, o), A = this.calcStartAngle(s, e, i, o);
    return Math.round(X(Math.asin(a / n)) + A + this.initialAngle);
  }, this.calcHypotenuse = (s, e, i, o) => I(s, e, i, o), this.calcCathetus = (s, e, i, o) => {
    if (s <= i && e <= o)
      return I(s, e, s, o);
    if (s >= i && e <= o)
      return I(s, e, i, e);
...
function _t(t) {
  this.resizeBox = t, this.subscriptions = {
    ...
    this.resizeBox.redraw(), h.emit(r.POINT_DRAG_END, this.resizeBox, d(s, { point: s.target })), h.emit(w.RESIZE_BOX_RESIZE, this.resizeBox, { oldPos: e, newPos: i });
  }, this.onLeftTopDragMove = (s) => {
...
function Mt(t) {
  this.shape = t, this.subscriptions = {
    ...
    if (e) {
      h.emit(w.RESIZE_BOX_RESIZE, e.resizeBox, { newPos: s.newPos, oldPos: s.oldPos });
      return;
    }
    const i = s.newPos.left - s.oldPos.left, o = s.newPos.top - s.oldPos.top;
    this.shape.moveBy(i, o);
    const [n, a] = this.shape.getMaxPointSize();
    this.shape.scaleTo(s.newPos.width - n * 2, s.newPos.height - a * 2), this.shape.redraw(), h.emit(w.RESIZE_BOX_RESIZE, this.shape, s);
  }, this.onRotate = (s) => {
    const e = this.shape.getRootParent(!0);
    if (e) {
      h.emit(T.ROTATE_BOX_ROTATE, e.rotateBox, { angle: s.angle });
      return;
    }
    this.shape.rotateBy(s.angle), this.shape.redraw(), h.emit(T.ROTATE_BOX_ROTATE, this.shape, s);
  }, this.mousedown = (s) => {
    ...
    const A = e.left + i, p = e.top + o, l = L(this.shape.root, !0), g = this.shape.getBounds();
...
function Pt() {
  this.draw = (t) => {
    ...
    const s = document.createElementNS("http://www.w3.org/2000/svg", "svg"), e = t.getPosition(t.options.groupChildShapes);
    ...
    return t.options.groupChildShapes && t.getChildren(!0).forEach((e) => {
      const i = e.svg.querySelector("defs");
      i && (s.innerHTML += i.innerHTML);
    }), s;
  }, this.addSvgPolygons = (t, s) => {
    const e = t.getPosition(t.options.groupChildShapes);
    if (t.svg) {
    ...
    t.options.groupChildShapes && t.getChildren(!0).forEach((i) => {
    ...
    t.calcPosition();
    const n = t.getPosition(t.options.groupChildShapes);
    [e, i] = H(e, i, n.width, n.height), t.scaleTo(e, i);
    const a = this.getSvg(t);
    for (let m of a.querySelectorAll("image"))
      if (m.getAttribute("href") && m.getAttribute("href").length) {
        const M = await k(await (await fetch(m.getAttribute("href"))).blob());
    ...
    const l = new Image(), g = new Blob([p], { type: "image/svg+xml" }), S = window.URL || window.webkitURL || window, _ = await k(g);
    l.addEventListener("load", () => {
      const m = document.createElement("canvas");
      l.width = e, l.height = i, m.width = l.width, m.height = l.height, m.getContext("2d").drawImage(l, 0, 0), S.revokeObjectURL(_);
      const P = m.toDataURL("image/png");
      if (s === U.BLOB) {
        o(q(P));
        return;
      }
...
function It() {
  this.shapes = [], this.activeShape = null, this.draggedShape = null, this.shapeOnCursor = null, this.containerEventListeners = [], this.init = () => (this.setEventListeners(), this), this.setEventListeners = () => {
    ...
    this.addContainerEvent(t.root, "mousemove", this.mousemove), this.addContainerEvent(t.root, "mouseup", this.mouseup, t.options.id), this.addContainerEvent(t.root, "dblclick", this.doubleclick), h.emit(Rt.MANAGER_ADD_CONTAINER_EVENT_LISTENERS, t.root);
...
function wt(t) {
  this.shape = t, this.contextMenu = null, this.updateContextMenu = () => {
    ...
    ], t.svg), t.options.canAddPoints && this.contextMenu.addItem("i" + t.guid + "_add_point", "Add Point", Et), this.displayGroupItems(), this.setEventListeners());
  }, this.setEventListeners = () => {
    ...
    s.options.groupChildShapes ? this.contextMenu.items.find((e) => e.id === "i" + this.shape.guid + "_ungroup") || (this.contextMenu.addItem("i" + this.shape.guid + "_ungroup", "Ungroup", vt), this.contextMenu.removeItem("i" + this.shape.guid + "_group")) : this.contextMenu.items.find((e) => e.id === "i" + this.shape.guid + "_group") || (this.contextMenu.removeItem("i" + this.shape.guid + "_ungroup"), this.contextMenu.addItem("i" + this.shape.guid + "_group", "Group", Ot));
...
function y() {
  this.root = null, this.points = [], this.svg = null, this.groupHelper = null, this.eventListener = new Mt(this), this.options = {
    id: "",
    name: "Unnamed shape",
    maxPoints: -1,
    stroke: "",
    ...
    return this.root = t, this.root.style.position = "relative", Object.assign(this, new wt(this)), this.setOptions(s), this.groupHelper = new Tt(this).init(), e && e.length && (this.setupPoints(e, Object.assign({}, this.options.pointOptions)), this.redraw()), this.eventListener.run(), typeof this.updateContextMenu == "function" && this.updateContextMenu(), this.applyDisplayMode(), (e && e.length || this.options.forceCreateEvent) && h.emit(r.SHAPE_CREATE, this, {}), this;
  }, this.setOptions = (t) => {
    !t || typeof t != "object" || (t.pointOptions = C(this.options.pointOptions, t.pointOptions), t.style = C(this.options.style, t.style), t.bounds = C(this.options.bounds, t.bounds), b(t.visible) && t.visible !== this.options.visible && (this.points.forEach((s) => s.options.visible = t.visible), this.resizeBox && this.resizeBox.setOptions({ shapeOptions: { visible: t.visible } }), this.rotateBox && this.rotateBox.setOptions({ shapeOptions: { visible: t.visible } })), this.options = C(this.options, t), this.points.forEach((s) => {
      s.setOptions(C({}, this.options.pointOptions)), s.options.bounds = this.getBounds(), s.options.zIndex <= this.options.zIndex && (s.options.zIndex = this.options.zIndex + 1), s.redraw();
    ...
    [t, s] = this.applyScaleRestriction(...H(t, s, i.width, i.height)), i.width >= 10 && t < 10 && (t = 10), i.height >= 10 && s < 10 && (s = 10);
    ...
    A && A.options.groupChildShapes && ([n, a] = A.getCenter(A.options.groupChildShapes)), s || (s = n), e || (e = a), this.initCenter && ([s, e] = this.initCenter), !(i && (!this.isInBounds(...R(t, o.left, o.top, s, e)) || !this.isInBounds(...R(t, o.right, o.top, s, e)) || !this.isInBounds(...R(t, o.left, o.bottom, s, e)) || !this.isInBounds(...R(t, o.right, o.bottom, s, e)))) && (this.points.forEach((p) => p.rotateBy(t, s, e)), this.options.groupChildShapes && this.getChildren(!0).forEach((p) => {
    ...
    return e && (i = i.map((o) => [o[0] + L(this.root).left, o[1] + L(this.root).top])), K(i, [t, s]);
  }, this.addEventListener = (t, s) => this.eventListener.addEventListener(t, s), this.removeEventListener = (t, s) => {
    this.eventListener.removeEventListener(t, s);
  }, this.show = () => {
    this.setOptions({ visible: !0 }), this.redraw();
  ...
  }, this.toSvg = () => v.toSvg(this), this.toPng = (t = U.DATAURL, s = null, e = null) => v.toPng(this, t, s, e), this.toJSON = (t = !0) => JSON.stringify(this.getJSON(t)), this.clone = (t = {}) => {
