...
function Jt(t) {
  this.rotateBox = t, this.subscriptions = {
    ...
    if (h === null)
      return;
    let l = h;
    this.previousAngle && (l -= this.previousAngle), this.previousAngle = h, r.emit(w.ROTATE_BOX_ROTATE, this.rotateBox, { angle: l });
...
function Kt(t) {
  this.resizeBox = t, this.subscriptions = {
    ...
    this.resizeBox.redraw(), r.emit(a.POINT_DRAG_END, this.resizeBox, A(e, { point: e.target })), r.emit(T.RESIZE_BOX_RESIZE, this.resizeBox, { oldPos: i, newPos: s });
  }, this.onLeftTopDragMove = (e) => {
...
function qt(t) {
  this.shape = t, this.subscriptions = {
    ...
    if (i) {
      r.emit(T.RESIZE_BOX_RESIZE, i.resizeBox, { newPos: e.newPos, oldPos: e.oldPos });
      return;
    }
    const s = e.newPos.left - e.oldPos.left, o = e.newPos.top - e.oldPos.top;
    this.shape.moveBy(s, o);
    const [n, h] = this.shape.getMaxPointSize();
    this.shape.scaleTo(e.newPos.width - n * 2, e.newPos.height - h * 2), this.shape.redraw(), r.emit(T.RESIZE_BOX_RESIZE, this.shape, e);
  }, this.onRotate = (e) => {
    const i = this.shape.getRootParent(!0);
    if (i) {
      r.emit(w.ROTATE_BOX_ROTATE, i.rotateBox, { angle: e.angle });
      return;
    }
    this.shape.rotateBy(e.angle), this.shape.redraw(), r.emit(w.ROTATE_BOX_ROTATE, this.shape, e);
  }, this.mousedown = (e) => {
...
function $t() {
  this.draw = (t) => {
    ...
    t.points.length < 1 || (t.options.hasContextMenu && t.shapeMenu && !t.shapeMenu.contextMenu && t.shapeMenu.updateContextMenu(), this.updateOptions(t), !e || !e.options.displayAsPath ? this.drawPolygon(t) : e && e.options.displayAsPath && e.guid !== t.guid && this.draw(e));
    ...
    const i = document.createElementNS("http://www.w3.org/2000/svg", "svg"), s = t.getPosition(e === null ? t.options.groupChildShapes : e);
    i.appendChild(this.getSvgDefs(t, e)), t.svg || this.draw(t), this.addSvgPolygons(t, i, e), i.setAttribute("xmlns", "http://www.w3.org/2000/svg");
    ...
    return (e === !0 || t.options.groupChildShapes && e !== !1) && t.getChildren(!0).forEach((s) => {
      if (s.svg || s.redraw(), s.svg) {
        const o = s.svg.querySelector("defs");
        o && (i.innerHTML += o.innerHTML);
      }
    }), i;
  }, this.addSvgPolygons = (t, e, i) => {
    const s = t.getPosition(i || t.options.groupChildShapes), o = [];
    if (t.svg || t.redraw(), t.svg && Array.from(t.svg.querySelectorAll("path")).forEach((n) => {
      if (n = n.cloneNode(), !t.options.groupChildShapes) {
        const h = t.points.map(
          (l) => "" + (l.x - s.left) + "," + (l.y - s.top)
        ).join(" ");
        n.setAttribute("d", "M " + h + " Z");
      }
      o.push({ polygon: n, zIndex: t.options.zIndex });
    }), i === !0 && !t.options.groupChildShapes && t.getChildren(!0).forEach((n) => {
      if (n.svg || n.redraw(), !n.svg && !t.options.groupChildShapes)
        return;
